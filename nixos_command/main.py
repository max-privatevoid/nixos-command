...
def activateNixosConfiguration(result, mode):
    switch = getOptionalRootCommand(
    ) + [os.path.join(result, "bin", "switch-to-configuration"), mode]
    printDebug(switch)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if (realiseNixosConfiguration(toplevel, nixargs, result)
            ...
            if not activateNixosConfiguration(result, mode):
                click.echo(
                    "warning: errors occured during configuration switch")
...
def buildCommand(installable, nixargs, attribute="config.system.build.toplevel"):
    toplevel = transform.normalizeNixosFlakeRef(
        installable, attribute=attribute)
...
def eval(installable, expr, nixargs):
    ...
    toplevel = transform.normalizeNixosFlakeRef(
        installable, attribute=f"config.{expr}")
